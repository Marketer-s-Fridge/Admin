...
import VisitorBarChart from "@/components/visitorBarChart";
...
export default function DashboardClient({
  ...
  const pubLoading = false;
  const userLoading = false;
  const postsLoading = false;
  const visitorLoading = false;
  const enquiryLoading = false;

  const pubErr = errors.publishedCnt;
  const userErr = errors.userCnt;
  const postsErr = errors.publishedPosts;
  const visitorErr = errors.visitorStats;
  const enquiryErr = errors.enquiries;
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 my-10">
                ...
                <ul className="space-y-3 text-sm">
                  {publishedPosts.map((post, index) => (
                    <li
                      ...
                      onClick={() =>
                        post.id && router.push(`/admin/contentsUpload/${post.id}`)
                      }
                    >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4">
                {[
                  { label: "오늘", value: visitorStats?.today },
                  { label: "이번 달", value: visitorStats?.thisMonth },
                  { label: "전체", value: visitorStats?.all },
                ].map(({ label, value }, i) => (
                  <div
                    key={i}
              ...
              <VisitorBarChart
                data={[
                  { label: "오늘", value: visitorStats?.today ?? 0 },
                  { label: "이번 달", value: visitorStats?.thisMonth ?? 0 },
                  { label: "전체", value: visitorStats?.all ?? 0 },
                ]}

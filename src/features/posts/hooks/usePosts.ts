...
export function usePosts(selectedCategory: string | null, isMock = true) {
  // ✅ 1. useQuery는 항상 호출되게 (단, isMock이면 비활성화)
  const { data: serverData, isLoading, error } = useQuery<PostResponseDto[]>({
    queryKey: ["posts", selectedCategory],
    queryFn: async () => await fetchPostsByStatus("PUBLISHED"),
    staleTime: 1000 * 60,
    enabled: !isMock, // ❗ isMock일 땐 요청 안 보냄
  });

  // ✅ 2. 데이터 소스 결정
  const baseData = isMock ? mockContents : serverData || [];

  // ✅ 3. 필터링 (카테고리 선택)
  const filtered = useMemo(() => {
    if (!selectedCategory) return baseData;
    return baseData.filter((item) =>
      item.title.toLowerCase().includes(selectedCategory.toLowerCase())
    );
  }, [baseData, selectedCategory]);
